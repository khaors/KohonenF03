...
! Overload grnd_array method (Added Oscar Garcia-Cabrejo)
    interface grnd_array
      module procedure grnd_array1
      module procedure grnd_array2
    end interface
!
 contains
...
!
  subroutine grnd_array1(values)
!  
    real(kind=8),dimension(:,:),intent(out) :: values
!
    integer :: nrow,ncol,ir,ic
!
    nrow=size(values,1);
    ncol=size(values,2);
!
    do ir=1,nrow
       ...
       enddo
    enddo
  end subroutine grnd_array1
!
  subroutine grnd_array2(values)
!  
    real(kind=8),dimension(:),intent(out) :: values
!
    integer :: nrow,ir
