!****h* Kohonen/two_level_self_organized_map_utilities
!
! NAME
...
!
implicit none;
!****c* two_level_self_organized_map_utilities/two_level_self_organized_map
! NAME
!   two_level_self_organized_map
! PURPOSE
!   Class to represent a two level self_organized_map
type,extends(kohonen_map_base) :: two_level_self_organized_map
  private
!
! ATTRIBUTES
! 
...
!*****   
 ...
 contains
!****f* two_level_self_organized_map_utilities/create
! NAME
!   create
! PURPOSE
...
!
   seed=training_parameters(1)%random_seed_(1)+100;
   call sgrnd(seed)
...
!              write(*,*) 'hexagonal'
              kohonen_map%coordinates(current_index,1)=kohonen_map%coordinates(current_index,1)+&
                                       .5d0*(mod(kohonen_map%coordinates(current_index,2),2.0d0));
              kohonen_map%coordinates(current_index,2)=(dsqrt(3.0d0)/2.d0)*kohonen_map%coordinates(current_index,2);
            endif
         enddo!iz
      enddo !iy
   enddo !ix
...
!
!
   seed=training_parameters(2)%random_seed_(1);
   call sgrnd(seed);
!
...
!****f* two_level_self_organized_map_utilities/destroy
! NAME
!   destroy
! PURPOSE
...
!****f* two_level_self_organized_map_utilities/train_2lsom
! NAME
!   train_2lsom
! PURPOSE
...
!****f* two_level_self_organized_map_utilities/predict_2lsom
! NAME
!   predict_2lsom
! PURPOSE
...
!*****
   ...
   real(kind=8),dimension(kohonen_map%number_variables1,kohonen_map%number_variables2) :: current_values
   real(kind=8),dimension(kohonen_map%number_clusters) :: distance_units
!
...
!****f* two_level_self_organized_map_utilities/train_grid_layer
! NAME
!   train_grid_layer
! PURPOSE
...
!*****
   ...
   real(kind=8) :: distortion,dist,dist_hit,maximum_radius,minimum_radius,current_radius,alpha
   real(kind=8) :: sigma2,geometric_distance2,h_neighborhood,real_distance,lambda,current_distance,d1,d2,d3
   real(kind=8) :: term3,distance_ratio
   type(kohonen_prototype) :: current_prototype,current_prototype1
   real(kind=8),dimension(kohonen_map%number_variables1,&
                kohonen_map%number_variables2) :: current_values,prototype_values,winner_values,term1,term2
   integer, allocatable :: pattern_index(:,:,:,:)
   logical :: testop
   integer :: unit_out
!
! determine best match grid unit
!
!       unit_out=199;
! !
...
!                write(unit_out,*) 'Grid Initial ',i,j,k
!                call kohonen_map%grid(i,j,k)%print(unit_out)
!             enddo
!          enddo
!       enddo
!       endif
...
!           kohonen_map%number_patterns(ihit,jhit)=kohonen_map%number_patterns(ihit,jhit)+1;
           ...
           ! define radius
           current_radius = max(maximum_radius*real(1001-iteration)/1000.0 + 0.9999999999,1.0d0);
           ! define learning rate
           alpha = max(kohonen_map%parameters(1)%learning_rate*(1.0d0-real(iteration)/1000.0),0.01d0);
           sigma2=current_radius**2
           !max(0.2*maximum_radius*(1.0d0-real(iteration)/1000.0),1.0d0);
           ! update prototypes
           if(debug_option .gt. 0) then
                 ...
                 select case(trim(kohonen_map%parameters(1)%som_type));
                   ...
                   case('visom');
                      ...
                      if( (ineigh .eq. ihit) .and. (jneigh .eq. jhit) .and. (kneigh .eq. khit) ) then                           
                         ...
                         prototype_values=prototype_values+h_neighborhood*(term1+term2*(distance_ratio-1.0d0)*term3);
                      endif
                      call kohonen_map%grid(ineigh,jneigh,kneigh)%set_prototype(prototype_values);                         
                  end select                     
             endif
...
!****f* two_level_self_organized_map_utilities/train_cluster_layer
! NAME
!   train_cluster_layer
! PURPOSE
...
!*****
   ...
   real(kind=8) :: dist_hit,dist,distortion,maximum_radius,minimum_radius,current_radius
   real(kind=8) :: alpha,h_neighborhood,sigma2,geometric_distance2
   real(kind=8),dimension(kohonen_map%number_variables1,&
                           kohonen_map%number_variables2) :: current_values,prototype_values
   real(kind=8),dimension(kohonen_map%number_variables1*kohonen_map%number_variables2) :: centers
   real(kind=8),dimension(kohonen_map%number_clusters) :: distance_units
   real(kind=8),dimension(kohonen_map%number_variables1*kohonen_map%number_variables2,&
      ...
      do iz=1,size(kohonen_map%grid,3);
         do iy=1,size(kohonen_map%grid,2);
            do ix=1,size(kohonen_map%grid,1);
              ...
              enddo!ic
              distortion = distortion + dist_hit;
              current_radius = max(maximum_radius*real(1001-iteration)/1000.0 + 0.9999999999,0.49d0);
              sigma2=current_radius**2;
              !current_radius=0.0d0;
              ! define learning rate
              alpha = max(kohonen_map%parameters(2)%learning_rate*&
                      (1.0d0-real(iteration)/1000.0),0.01d0);
...
!****f* two_level_self_organized_map_utilities/print_2lsom
! NAME
!   print_2lsom
! PURPOSE
...
!****f* two_level_self_organized_map_utilities/set_cluster_layer
! NAME
!   set_cluster_layer
! PURPOSE
...
!****f* two_level_self_organized_map_utilities/set_parameters
! NAME
!   set_parameters
! PURPOSE
...
!****f* two_level_self_organized_map_utilities/evaluate_2lsom
! NAME 
!   evaluate_2lsom
! PURPOSE
...
!*****
   ...
   real(kind=8) :: current_dissimilarity
   real(kind=8),dimension(size(input_data)) :: mean_dissimilarity_a,silhouette,min_b,W,B
   real(kind=8),dimension(size(input_data),kohonen_map%number_clusters) :: mean_dissimilarity_b
...
!               endif
      ...
      if(mean_dissimilarity_a(ipattern1) .lt. min_b(ipattern1)) then
        silhouette(ipattern1)=1.0d0-(mean_dissimilarity_a(ipattern1)/min_b(ipattern1));
      else
        silhouette(ipattern1)=(min_b(ipattern1)/mean_dissimilarity_a(ipattern1))-1.0d0;
      endif
...
!****f* two_level_self_organized_map_utilities/calculate_sum2_clusters_grid
! NAME
...
!*****
  ...
  real(kind=8),dimension(kohonen_map%number_clusters) :: W,B
  real(kind=8) :: current_dissimilarity
  real(kind=8),dimension(kohonen_map%number_variables1*&
  ...
  real(kind=8),dimension(kohonen_map%number_nodes) :: mean_dissimilarity_a,min_b,silhouette
  real(kind=8),dimension(kohonen_map%number_nodes,kohonen_map%number_clusters) :: mean_dissimilarity_b
...
!  find samples in each cluster
   min_b=10.0d8;
!   
   B=0.0d0;W=0.0d0;mean_unit=0.0d0
!
  do iz1=1,size(kohonen_map%grid,3);
     do iy1=1,size(kohonen_map%grid,2);
        do ix1=1,size(kohonen_map%grid,1);
...
!
          if(mean_dissimilarity_a(current_pos) .lt. min_b(current_pos)) then
            silhouette(current_pos)=1.0d0-(mean_dissimilarity_a(current_pos)/min_b(current_pos));
          else
            silhouette(current_pos)=(min_b(current_pos)/mean_dissimilarity_a(current_pos))-1.0d0;
          endif
!
        enddo!ix1
     enddo!iy1
...
!****f* two_level_self_organized_map_utilities/calculate_cluster_measures
! NAME
...
!****f* two_level_self_organized_map/read_som_layer
! NAME
!   read_som_layer
! PURPOSE
...
!*****
   ...
   real(kind=8),dimension(kohonen_map%parameters(1)%number_variables1,&
   ...
   do ic=1,size(kohonen_map%cluster_layer);
       ...
       do ix=1,size(centers,1);
          do iy=1,size(centers,2);
             if(input_pattern(ix,iy) > 0.0d0) then
...
!****f* two_level_self_organized_map_utilities/get_cluster_samples
! NAME
!   get_cluster_samples
! PURPOSE
...
!****f* two_level_self_organized_map_utilities/position2index
! NAME
!   position2index
! PURPOSE
...
!****f* two_level_self_organized_map_utilities/index2position
! NAME
!   index2position
! PURPOSE
...
!****f* two_level_self_organized_map_utilities/calculate_distance_matrix
! NAME
...
!****f*  two_level_self_organized_map_utilities/calculate_coordinates
! NAME
...
!
  ...
  !****f* two_level_self_organized_map/find_best_match_unit
! NAME
...
!
 ...
 !alpha = max(kohonen_map%parameters%learning_rate*(1.0d0-real(iteration)/1000.0),0.01d0);
  alpha = max(kohonen_map%parameters(1)%learning_rate*time_factor,0.01d0);
  sigma2=current_radius**2
!
  do ic=1,size(kohonen_map%coordinates,1)
     current_pos=position2index(ihit,jhit,khit,nx,ny);
...
!****f* two_level_self_organized_map_utilities/calculate_distance_between_prototypes
! NAME
...
!****f* two_level_self_organized_map/calculate_u_matrix
! NAME
!   calculate_u_matrix
! PURPOSE
...
!****f* two_level_self_organized_map/assign_input_to_clusters
! NAME
